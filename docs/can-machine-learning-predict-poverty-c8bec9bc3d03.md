...
数据集的另一个重要属性是+ve 和-ve 类之间的不平衡(非贫困人口远远超过贫困人口)。对于国家 A，数据仍然是平衡的，但是对于 B 和 C，数据具有非常偏斜的分布。为了在这种倾斜的数据上训练模型，我们使用 Python 中的[不平衡学习库](http://contrib.scikit-learn.org/imbalanced-learn/stable/)尝试了不同的方法:
