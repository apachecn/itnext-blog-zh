...
没错。几乎所有的异步都与`setTimeout`有关。甚至`setInterval`都可以用`setTimeout`建造。它们被称为调度器，因为它们允许您将它们的执行时间指定为“在这个函数之后的某个时间”，一直到整数 max。
